...
==============================================================================
*/}}
{{- define "eda-kpis.podTemplateSpec" -}}
metadata:
  labels:
{{ include "eda-kpis.podLabels" . | indent 4 }}
  {{- $checksums := include "eda-kpis.configChecksum" (dict "ctx" .) }}
...
{{ $checksums | indent 4 }}
  ...
  {{- end }}
spec:
  serviceAccountName: {{ include "eda-kpis.serviceAccountName" . }}
...
{{ toYaml .root.Values.global.imagePullSecrets | indent 4 }}
  ...
  {{- end }}
{{ include "eda-kpis.podSecurityContext" . | indent 2 }}
  {{- if .svc.nodeSelector }}
  nodeSelector:
...
{{ toYaml .svc.tolerations | indent 4 }}
  {{- end }}
{{ include "eda-kpis.affinity" . | indent 2 }}
{{ include "eda-kpis.topologySpreadConstraints" . | indent 2 }}
...
{{ toYaml .svc.initContainers | indent 4 }}
  ...
  containers:
    - name: {{ .serviceName }}
      image: {{ include "eda-kpis.image" . }}
      imagePullPolicy: {{ .svc.image.pullPolicy | default "IfNotPresent" }}
{{ include "eda-kpis.containerSecurityContext" . | indent 6 }}
...
{{ toYaml (.svc.envFrom | default .root.Values.global.envFrom) | indent 8 }}
      {{- end }}
{{ include "eda-kpis.resources" . | indent 6 }}
{{ include "eda-kpis.probe" (merge (dict "kind" "startupProbe") .) | indent 6 }}
{{ include "eda-kpis.probe" (merge (dict "kind" "readinessProbe") .) | indent 6 }}
{{ include "eda-kpis.probe" (merge (dict "kind" "livenessProbe") .) | indent 6 }}
      {{- if .svc.lifecycle }}
...
{{ toYaml .svc.volumes | indent 4 }}
    ...
    - name: config
      configMap:
        name: {{ .svc.configMap.nameOverride | default (include "eda-kpis.serviceFullname" (dict "root" .root "serviceName" .serviceName)) }}
    {{- end }}
    {{- if .svc.secret.enabled }}
    - name: secret
      secret:
        secretName: {{ .svc.secret.nameOverride | default (include "eda-kpis.serviceFullname" (dict "root" .root "serviceName" .serviceName)) }}
    {{- end }}
  {{- end }}
{{- end -}}
