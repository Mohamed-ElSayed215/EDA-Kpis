...
- `values.yaml` defines `global` (defaults) and `services` (a map of
  ...
  `templates/_helpers.tpl` (`eda-kpis.mergedService`).
...
1. Add an entry under `services.<name>` in `helm/eda-kpis/values.yaml`
...
helm lint helm/eda-kpis

# Render each environment and eyeball the diff
helm template eda-kpis-local helm/eda-kpis -f environments/local/values.yaml
helm template eda-kpis-dev   helm/eda-kpis -f environments/dev/values.yaml
helm template eda-kpis-stage helm/eda-kpis -f environments/stage/values.yaml
helm template eda-kpis-prod  helm/eda-kpis -f environments/prod/values.yaml
...
`helm lint --strict` on every PR that touches `helm/eda-kpis/**` or
`environments/**`.
