...
helm.sh/chart: {{ include "portal.chart" .root }}
...
{{- .svc.serviceAccount.name | default (include "portal.serviceFullname" (dict "root" .root "serviceName" .serviceName)) -}}
{{- else -}}
...
==============================================================================
*/}}
{{- define "portal.podSecurityContext" -}}
...
{{- define "portal.containerSecurityContext" -}}
  ...
  podAntiAffinity:
    ...
    requiredDuringSchedulingIgnoredDuringExecution:
      - topologyKey: {{ $svc.podAntiAffinity.topologyKey | default "kubernetes.io/hostname" }}
        labelSelector:
          matchLabels:
{{ include "portal.selectorLabels" . | indent 12 }}
    {{- else }}
    preferredDuringSchedulingIgnoredDuringExecution:
      - weight: {{ $svc.podAntiAffinity.weight | default 100 }}
        podAffinityTerm:
          ...
          labelSelector:
            matchLabels:
{{ include "portal.selectorLabels" . | indent 14 }}
    {{- end }}
{{- end }}
{{- end }}
{{- end -}}

{{- define "portal.topologySpreadConstraints" -}}
...
{{- range .svc.topologySpreadConstraints }}
  - maxSkew: {{ .maxSkew | default 1 }}
    ...
    labelSelector:
      matchLabels:
{{ include "portal.selectorLabels" $ | indent 8 }}
{{- end }}
{{- end }}
{{- end -}}
...
==============================================================================
*/}}
{{- define "portal.configChecksum" -}}
{{- $ctx := .ctx -}}
{{- if $ctx.svc.configMap.enabled }}
checksum/configmap: {{ include "portal.configmapData" $ctx | sha256sum }}
{{- end }}
{{- if $ctx.svc.secret.enabled }}
checksum/secret: {{ include "portal.secretData" $ctx | sha256sum }}
{{- end }}
{{- end -}}

{{- define "portal.configmapData" -}}
{{- toYaml (.svc.configMap.data | default dict) }}
{{- toYaml (.svc.configMap.binaryData | default dict) }}
{{- end -}}

{{- define "portal.secretData" -}}
